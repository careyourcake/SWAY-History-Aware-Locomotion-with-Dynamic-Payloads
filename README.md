...
实验顺序固定为 B0 sanity（无负载固定速度）、B1 locomotion baseline（无负载随机命令）、B2 static baseline、B3 dynamic MLP baseline；全部通过验收后才运行 GRU/frame-stack 消融。旧 `runs/gpu_matrix` 仅作为失败诊断，不纳入主结果。

各阶段配置位于 `configs/b0_sanity.yaml`、`b1_locomotion.yaml`、`b2_static.yaml` 和 `b3_dynamic_mlp.yaml`。先运行 B0：

```bash
python -m b2_mjx.train --config configs/b0_sanity.yaml --run-dir runs/baseline/b0/seed_0
python -m b2_mjx.evaluate --checkpoint runs/baseline/b0/seed_0 --scenario unloaded --episodes 100
python scripts/check_stage.py runs/baseline/b0/seed_0/evaluation/metrics.csv
```

只有门禁输出 `PASS` 才进入下一阶段。训练结束会在 run 目录写入 `best_checkpoint.json`；评估命令可直接接收 run 目录，并自动载入该最佳 checkpoint，而非最后一个 checkpoint。

服务器上可按阶段运行 `bash scripts/run_baselines.sh B0 0`，依次完成 B0、B1、B2、B3；脚本在评估门禁失败时以非零状态停止。三个 baseline seed 全部通过后，再分别运行 `bash scripts/run_ablation.sh 0`、`1`、`2`。静态 B2 使用自己的 1–5 kg 配置，不再通过旧矩阵的 `mlp_static` 覆盖项启动。
...
```bash
bash scripts/run_curriculum.sh mlp_dynamic 0 runs/curriculum
bash scripts/run_curriculum.sh stack5_dynamic 0 runs/curriculum
bash scripts/run_curriculum.sh gru_dynamic 0 runs/curriculum
```
